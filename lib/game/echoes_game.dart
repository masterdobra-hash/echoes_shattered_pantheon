...
/// - Combat: real-time skills with cooldowns, AOE telegraphs
class EchoesGame extends FlameGame with TapCallbacks {
  ...
  Future<void> onLoad() async {
    ...
    try {
      ...
      FlameAudio.bgm.play('bgm_ruined_olympus.mp3', volume: 0.45);
    } catch (_) {
      _audioReady = false;
    }

    await add(GroundTileLayer(size: worldSize));
  ...
  void update(double dt) {
    ...
    // Trash AI: chase player and deal melee damage on contact
    for (final mob in trash) {
      ...
      if (canMove && dist > 30) {
        ...
        // melee tick ~ once per 1.2s using flashTimer as ad-hoc lock
        // We use a tiny attack timer via burnTimer slot? Keep simple: random damage per second-ish
  ...
  Future<void> _spawnBossTelegraphAndStrike(Vector2 center, double radius, int damage) async {
    await add(TelegraphComponent(position: center, radius: radius, lifetime: 0.9));
    if (_audioReady) FlameAudio.play('sfx_boss_telegraph.mp3', volume: 0.7);
    // After windup, deal damage if player is still inside the zone
    Future.delayed(const Duration(milliseconds: 900), () {
      if (!player.isAlive) return;
      if (player.position.distanceTo(center) <= radius) {
        player.takeDamage(damage);
        combatLog.value = 'Telegraph hits! -$damage HP';
        add(HitFlashComponent(position: player.position, color: const Color(0xFFFF3030), radius: 50));
      }
    });
  }
  ...
  Future<bool> castSkill(String skillId) async {
    ...
    add(HitFlashComponent(position: player.position, color: impactColor, radius: result.radius * 0.6));
    ...
    // Switch BGM at phase 3 (boss enrage)
    if (_audioReady && boss.isAlive && boss.phase == 3 && FlameAudio.bgm.audioPlayer.state.toString().contains('playing')) {
      FlameAudio.bgm.stop();
      FlameAudio.bgm.play('bgm_fallen_hoplite.mp3', volume: 0.5);
    }
    return true;
  }
